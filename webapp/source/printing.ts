...
export function printEmptyJournal() {
    let parts = [];
    const project = Globals.project();
    const panelSpecificationCollection = Globals.panelSpecificationCollection();
    const allPages = panelSpecificationCollection.buildListOfPages();
    ...
    parts.push('<div class=\"narrafirma-emptyjournal-title\">PNI Project Journal</div>');
    parts.push(`
        <div class=\"narrafirma-emptyjournal-intro\">
        </p>This HTML file was generated by 
        <a href="https://www.narrafirma.com">NarraFirma</a>, a software package that supports facilitators of
        <a href="https://www.workingwithstories.org">Participatory Narrative Inquiry</a> (PNI).
        This file includes every journal-type question in NarraFirma, 
        ...
        <ul><li>
        To use this file on a computer, save it and open it in (or copy and paste its contents into) a word processor.
        Then type in your answers after each question, adding more space as you need it.
        </li><li>
        To prepare this file for use without a computer, 
        save it and open it in (or copy and paste its contents into) a word processor.
        Add in several blank spaces after each question for people to write in. Then print the file.
        </li></ul>
        <p>
        Feel free to remove, add to, or change any of the instructions or questions in this file to suit your unique project.
        For example, you might want to replace these starting instructions with the name of your project
        and your project synopsis (once you've written it).
        </p>
        <p>
        For more information on Participatory Narrative Inquiry, 
        visit <a href="https://www.workingwithstories.org">workingwithstories.org</a>.
        For more information on NarraFirma, visit <a href="https://www.narrafirma.com">narrafirma.com</a>.
        </p>
        <p><i>Generated by NarraFirma version ` + versions.narrafirmaApplication + " on "  
        + new Date().toLocaleDateString().toString() + ".</p></i></div>"
        );

    allPages.forEach(function(page) {
        if (page.section === "dashboard" || page.section === "administration") return;
        if (pagesToSkip.indexOf(page.id) >= 0) return;

        if (page.isHeader) { // phase
            parts.push("<div class=\"narrafirma-emptyjournal-phasename\">" + emptyJournalName(page) + "</div>");
            // assumption that first field on phase-dashboard page is label describing it
            if (page.panelFields.length > 0) {
                const pageLabel = emptyJournalPrompt(page.panelFields[0]);
                parts.push("<div class=\"narrafirma-emptyjournal-phasenamepage-firstlabel\">" + pageLabel + "</div>");
            }
        } else {
            parts.push("<div class=\"narrafirma-emptyjournal-pagename\">" + emptyJournalName(page) + "</div>");
            const pageParts = printEmptyJournalForPage(page, displayTypesNotToShow, panelSpecificationCollection);
            if (pageParts) parts = parts.concat(pageParts);
        }
    });
    const html = generateHTMLForPage("Report - " + project.projectNameOrNickname(), null, cssForEmptyJournal(), null, parts.join("\n"));
    printHTML(html);
}

function printEmptyJournalForPage(page, displayTypesNotToShow, panelSpecificationCollection) {
    let parts = [];
    if (page.panelFields.length > 0) {
        let pageLabel = emptyJournalPrompt(page.panelFields[0]); 
        // if emptyJournalPrompt of FIRST LABEL on page BEGINS with three asterisks,
        // it means to print that label then DO NOT print the rest of the page
        if (pageLabel.indexOf("***") === 0) {
            pageLabel = pageLabel.slice(3);
            parts.push("<div class=\"narrafirma-emptyjournal-firstlabel\">" + pageLabel + "</div>");
            return parts;
        } else {
            page.panelFields.forEach(function(field) {
                if (displayTypesNotToShow.indexOf(field.displayType) >= 0) return;
                if (field.displayClass === "narrafirma-reminder") return;
                let textToWrite = emptyJournalPrompt(field); 
                if (textToWrite === "") return; // blank string means hide field

                if (field.displayType === "header") { // header within page
                    parts.push("<div class=\"narrafirma-emptyjournal-header\">" + textToWrite + "</div>");
                } else if (field.displayType === "label") {
                    let classToUse = "narrafirma-emptyjournal-label"
                    if (page.panelFields.indexOf(field) === 0) {
                        classToUse = "narrafirma-emptyjournal-firstlabel"
                    }
                    parts.push('<div class="' + classToUse + '">' + textToWrite + "</div>");
                } else if (field.displayType === "grid") {
                    const gridParts = printEmptyJournalForGrid(field, panelSpecificationCollection, displayTypesNotToShow);
                    if (gridParts) parts = parts.concat(gridParts);
                } else {
                    parts.push('<div class=\"narrafirma-report-question-prompt\">' + textToWrite + "</div>");
                }
            });
        }
    }
...
function printEmptyJournalForGrid(field, panelSpecificationCollection, displayTypesNotToShow) {
    let parts = [];
    const gridPanel = panelSpecificationCollection.getPanelSpecificationForPanelID(field.displayConfiguration.itemPanelID);
    if (gridPanel) {
        parts.push("<div class=\"narrafirma-emptyjournal-grid-question-prompt\">" + emptyJournalPrompt(field) + "</div>");
        parts.push("<hr class=\"narrafirma-emptyjournal-grid-start-stop\">");
        gridPanel.panelFields.forEach(function(gridField) {
            if (displayTypesNotToShow.indexOf(gridField.displayType) >= 0) return;
            if (gridField.displayType === "grid") {
                const gridParts = printEmptyJournalForGrid(gridField, panelSpecificationCollection, displayTypesNotToShow);
                if (gridParts) parts = parts.concat(gridParts);
            } else {
                let textToWrite = emptyJournalPrompt(gridField); 
                if (textToWrite === "") return; // blank string means hide field
                if (gridField.displayType === "header") {
                    parts.push("<div class=\"narrafirma-emptyjournal-header\">" + textToWrite + "</div>");
                } else if (field.displayType === "label") {
                    parts.push("<div class=\"narrafirma-emptyjournal-label\">" + textToWrite + "</div>");
                } else {
                    parts.push("<div class=\"narrafirma-emptyjournal-question-prompt\">" + textToWrite + "</div>");
                }
            }
            });
        }
...
function emptyJournalPrompt(field) {
    return (field.emptyJournalPrompt != undefined) ? field.emptyJournalPrompt : field.displayPrompt;
}

function emptyJournalName(page) {
    return (page.emptyJournalName != undefined) ? page.emptyJournalName : page.displayName;
}

function cssForEmptyJournal() {
    const result = `div {
        margin: 0.5em 0 0.5em 0;
        padding: 0.2em 0 0.2em 0;
    }
    
.narrafirma-emptyjournal-title {
    font-size: 2rem;
    background: var(--color-header);
}

.narrafirma-emptyjournal-intro {
    font-size: 0.9rem;
}

.narrafirma-emptyjournal-phasename {
    font-size: 1.5em;
    font-weight: bold;
    background-color: lightgray;
...
.narrafirma-emptyjournal-phasenamepage-firstlabel {
    font-style: italic;
    margin: 0 0 0.5em 0;
    padding-left: 0.5em;
    background-color: lightgray;
...
.narrafirma-emptyjournal-pagename {
    font-size: 1.1em;
    font-weight: bold;
    padding: 0.25em;
...
.narrafirma-emptyjournal-header {
    font-style: italic;
}

.narrafirma-emptyjournal-firstlabel {
    font-style: italic;
}

.narrafirma-emptyjournal-label {
    font-size: 1rem;
}

.narrafirma-emptyjournal-grid-start-stop {
    width: 100%;
    border-style: none none dotted; 
}

.narrafirma-emptyjournal-grid-question-prompt {
    font-style: italic;
}
     `;
    return result;
