...
export function importEntireProject() {
    ...
    chooseProjectFileToImport((contents) => {
        ...
        const progressModel = dialogSupport.openProgressDialog("Importing project messages...", "Progress importing project messsages", "Cancel", dialogCancelled);
...
export function importProjectCurrentState() {
    ...
    chooseProjectFileToImport((contents) => {
        ...
        if (importObject.storyCollections) {
            for (let storyCollectionName in importObject.storyCollections) {
