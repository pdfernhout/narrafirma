...
const journalName = m.prop("");
const userName = m.prop("");
const userPassword = m.prop("");
const roleName = m.prop("");
const topicName = m.prop("");
...
function messageReceived(message)  {
    if (message._topicIdentifier && message._topicIdentifier.type === "userInformation") {
        ...
        // TODO: Queue a redraw now, but might want to wait until idle instead...
        m.redraw();
    }
}
