...
function positionForQuestionAnswer(question, answer, unansweredText) {
    ...
    const answerCount = options.length;
    ...
    const position = 100 * answerIndex / (options.length - 1);
    return position;  
}
...
function makePlotItem(xAxisQuestion, yAxisQuestion, xValue, yValue, story, unansweredText) {
    ...
    const x = positionForQuestionAnswer(xAxisQuestion, xValue, unansweredText);
    const y = positionForQuestionAnswer(yAxisQuestion, yValue, unansweredText);
...
// TODO: Put ellipsis starting between words so no words are cut off
...
function addXAxisLabel(chart, label, options: AxisOptions) {
    ...
    let xPosition;
    let yPosition = chart.fullHeight - 16;
...
function addStatisticsPanelForChart(chartPane: HTMLElement, graphHolder: GraphHolder, statistics, chartTitle, chartSize, hide = false) {
    ...
    if (statistics.statsSummary.substring("None") === 0 || statistics.statsDetailed.length !== 0) {
        ...
        if (statistics.allResults) {
            ...
            for (const resultKeyIndex in sortedResultKeys) {
...
function updateSelectedStories(chart, storyDisplayItemsOrClusters, graphHolder: GraphHolder, storiesSelectedCallback, selectionTestFunction) {
    ...
    storyDisplayItemsOrClusters.classed("selected", function(plotItem) {
        const selected = selectionTestFunction(extent, plotItem);
        let story;
        if (selected) {
            if (plotItem.stories) {
...
export function d3BarChartForValues(graphHolder: GraphHolder, plotItems, xLabels, chartTitle, xAxisLabel, question, storiesSelectedCallback, hideStatsPanel = false) {
    ...
    const letterSize = 6; // it would be better to get this from the DOM - but it would decrease performance...
    const margin = {top: 20, right: 15, bottom: 90 + longestLabelTextLength * letterSize, left: 70};
...
export function d3HistogramChartForDataIntegrity(graphHolder: GraphHolder, scaleQuestions, dataIntegrityType) {
    ...
    if (dataIntegrityType == "All scale values") {
        for (const storyIndex in stories) {
            const story = stories[storyIndex];
            for (const questionIndex in scaleQuestions) {
        ...
        for (const participantID in storiesByParticipant) {
            const valuesForParticipant = [];
            for (const storyIndex in storiesByParticipant[participantID]) {
            ...
            if (dataIntegrityType == "Participant means") {
                if (valuesForParticipant.length > 0) {
                    mean = jStat.mean(valuesForParticipant);
...
export function d3ContingencyTable(graphHolder: GraphHolder, xAxisQuestion, yAxisQuestion, scaleQuestion, storiesSelectedCallback, hideStatsPanel = false) {
    ...
    for (const index in stories) {
        ...
        if (xValue !== null && yValue !== null) {
            ...
            if (!xHasCheckboxes && !yHasCheckboxes) {
                ...
                for (const xIndex in xValues) {incrementMapSlot(results, JSON.stringify({x: xValues[xIndex]}));}
                for (const yIndex in yValues) {incrementMapSlot(results, JSON.stringify({y: yValues[yIndex]}));}
                
                for (const xIndex in xValues) {
                    for (const yIndex in yValues) {
    ...
    for (const columnName in columnLabels) {
    ...
    let longestRowText = "";
    for (const rowName in rowLabels) {
    ...
    for (const columnIndex in columnLabelsArray) {
        const column = columnLabelsArray[columnIndex];
        for (const rowIndex in rowLabelsArray) {
            ...
            if (scaleQuestion) {
                ...
                for (let i = 0; i < storiesForNewPlotItem.length; i++) {
                    const story = storiesForNewPlotItem[i];
                    const scaleValue = parseInt(story.fieldValue(scaleQuestion.id));
                    if (scaleValue) {
                        scaleValues.push(scaleValue);
                    }
                }
                mean = jStat.mean(scaleValues);
                if (!isNaN(mean)) {
    ...
    const letterSize = 6; // it would be better to get this from the DOM - but it would decrease performance...
    const margin = {top: 20, right: 15, bottom: 90 + longestColumnTextLength * letterSize, left: 90 + longestRowTextLength * letterSize};
    ...
    const xAxis = addXAxis(chart, xScale, {labelLengthLimit: labelLengthLimit, drawLongAxisLines: true, rotateAxisLabels: true, graphType: "table", namesAndTotals: columnNamesAndTotals});
    ...
    const yAxis = addYAxis(chart, yScale, {labelLengthLimit: labelLengthLimit, drawLongAxisLines: true, graphType: "table", namesAndTotals: rowNamesAndTotals});
    ...
    let storyDisplayClusters
    let meanRects
    let sdRects

    if (scaleQuestion) {

        let yValueMultiplier
        if (maxPlotItemValue === 0) {
            yValueMultiplier = 0;
        } else {
            yValueMultiplier = yScale.rangeBand() / maxPlotItemValue;
        }
        const barWidth = xScale.rangeBand();
    
        // rectangles
        const storyDisplayClusters = chartBody.selectAll(".contingencyChart-miniHistogram")
