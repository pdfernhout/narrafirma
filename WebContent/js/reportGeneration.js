define(["require", "exports", "js/applicationWidgets/add_quizScoreResult", "js/panelBuilder/translate"], function (require, exports, add_quizScoreResult, translate) {
    ...
    function calculate_report(panelSpecificationCollection, model, headerPageID) {
        ...
        if (!panelSpecificationCollection)
            return "ERROR: in calculate_report, panelSpecificationCollection is not set";
        var report = "<br><br>";
        ...
        for (var pageIndex in pageList) {
            ...
            if (panelDefinition.displayType !== "page")
                continue;
            report += "<div>";
            ...
            if (questionsAnsweredCount === 0)
                report += translate("#no_questions_answered_on_page", "(No questions answered on this page)");
            report += "</div><br>";
        }
        return report;
    }
    ...
    function labelForQuestion(question) {
        ...
        if (!shortName)
            shortName = translate(question.id + "::prompt", "");
        if (!shortName)
            shortName = question.displayName;
        if (!shortName)
            shortName = question.displayPrompt;
        if (!shortName) {
    ...
    function displayStringForValue(question, value, level) {
        ...
        if (value instanceof Array) {
            ...
            if (question !== null && (question.displayType === "select" || question.displayType === "radiobuttons"))
