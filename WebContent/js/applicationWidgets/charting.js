define(["require", "exports", "d3", "dojo/dom-construct", "dijit/layout/ContentPane"], function (require, exports, d3, domConstruct, ContentPane) {
    ...
    function createBrush(chartBody, xScale, yScale, brushendCallback) {
        ...
        var brush = d3.svg.brush().x(xScale).on("brushend", brushendCallback);
        if (yScale)
            brush.y(yScale);
        var brushGroup = chartBody.append("g").attr("class", "brush").call(brush);
        if (!yScale) {
            brushGroup.selectAll("rect").attr("y", 0).attr("height", chartBody.attr("height"));
        }
    ...
    function addXAxis(chart, xScale, configure) {
        ...
        if (!configure.rotateAxisLabels) {
            chart.chartBody.append('g').attr('transform', 'translate(0,' + chart.height + ')').attr('class', 'x axis').call(xAxis);
        }
        else {
            chart.chartBody.append('g').attr('transform', 'translate(0,' + chart.height + ')').attr('class', 'x axis').call(xAxis).call(xAxis).selectAll("text").style("text-anchor", "end").attr("dx", "-0.8em").attr("dy", "0.15em").attr("transform", function (d) {
                return "rotate(-65)";
            });
        }
        return xAxis;
    ...
    function addYAxis(chart, yScale, configure) {
        ...
        chart.chartBody.append('g').attr('class', 'y axis').call(yAxis);
        return yAxis;
    }
    function addXAxisLabel(chart, label, labelLengthLimit) {
        ...
        if (label.length > labelLengthLimit) {
            shortenedLabelSVG.append("svg:title").text(label);
        }
    }
    ...
    function d3BarChart(graphBrowserInstance, question, storiesSelectedCallback) {
        ...
        // draw the x axis
        var xScale = d3.scale.ordinal().domain(xLabels).rangeRoundBands([0, chart.width], 0.1);
        chart.xScale = xScale;
        chart.xQuestion = question;
        var xAxis = addXAxis(chart, xScale, { labelLengthLimit: 9 });
        addXAxisLabel(chart, nameForQuestion(question));
        // draw the y axis
        var maxItemsPerBar = d3.max(allPlotItems, function (plotItem) {
            return plotItem.value;
        });
        var yScale = d3.scale.linear().domain([0, maxItemsPerBar]).range([chart.height, 0]);
        chart.yScale = yScale;
        // Extra version of scale for calculating heights without subtracting as in height - yScale(value)
        var yHeightScale = d3.scale.linear().domain([0, maxItemsPerBar]).range([0, chart.height]);
        ...
        var bars = chartBody.selectAll(".bar").data(allPlotItems).enter().append("g").attr("class", "bar").attr('transform', function (plotItem) {
            return 'translate(' + xScale(plotItem.name) + ',' + yScale(0) + ')';
        });
        var barBackground = bars.append("rect").attr("x", function (plotItem) {
            return 0;
        }).attr("y", function (plotItem) {
            return 0;
        }).attr("height", function (plotItem) {
            return yHeightScale(plotItem.value);
        }).attr("width", xScale.rangeBand());
        // Overlay stories on each bar...
        var storyDisplayItems = bars.selectAll(".story").data(function (plotItem) {
            return plotItem.stories;
        }).enter().append("rect").attr('class', function (d, i) {
            return "story " + ((i % 2 === 0) ? "even" : "odd");
        }).attr("x", function (plotItem) {
            return 0;
        }).attr("y", function (plotItem, i) {
            return yHeightScale(-i - 1);
        }).attr("height", function (plotItem) {
            return yHeightScale(1);
        }).attr("width", xScale.rangeBand());
        // Add tooltips
        storyDisplayItems.append("svg:title").text(function (storyItem) {
            var story = storyItem.story;
            var tooltipText = "Title: " + story.__survey_storyName + "\n" + nameForQuestion(question) + ": " + displayTextForAnswer(story[question.id]) + "\nText: " + limitStoryTextLength(story.__survey_storyText);
            return tooltipText;
        });
    ...
    function d3HistogramChart(graphBrowserInstance, scaleQuestion, choiceQuestion, choice, storiesSelectedCallback) {
        ...
        var chartBody = chart.chartBody;
        // Draw the x axis
        var xScale = d3.scale.linear().domain([0, 100]).range([0, chart.width]);
        chart.xScale = xScale;
        ...
        var yScale = d3.scale.linear().domain([0, maxValue]).range([chart.height, 0]);
        chart.yScale = yScale;
        chart.subgraphQuestion = choiceQuestion;
        chart.subgraphChoice = choice;
        // Extra version of scale for calculating heights without subtracting as in height - yScale(value)
        var yHeightScale = d3.scale.linear().domain([0, maxValue]).range([0, chart.height]);
        ...
        // Append brush before data to ensure titles are drown
        chart.brush = createBrush(chartBody, xScale, null, brushend);
        var bars = chartBody.selectAll(".bar").data(data).enter().append("g").attr("class", "bar").attr("transform", function (d) {
        ...
        // Add tooltips
        storyDisplayItems.append("svg:title").text(function (plotItem) {
    ...
    function d3ScatterPlot(graphBrowserInstance, xAxisQuestion, yAxisQuestion, storiesSelectedCallback) {
        ...
        // draw the y axis
        var yScale = d3.scale.linear().domain([0, 100]).range([chart.height, 0]);
        chart.yScale = yScale;
        ...
        var storyDisplayItems = chartBody.selectAll(".story").data(allPlotItems).enter().append("circle").attr("class", "story").attr("r", 8).attr("cx", function (plotItem) {
            return xScale(plotItem.x);
        }).attr("cy", function (plotItem) {
            return yScale(plotItem.y);
        });
        // Add tooltips
        storyDisplayItems.append("svg:title").text(function (plotItem) {
            var tooltipText = "Title: " + plotItem.story.__survey_storyName + "\nX (" + nameForQuestion(xAxisQuestion) + "): " + plotItem.x + "\nY (" + nameForQuestion(yAxisQuestion) + "): " + plotItem.y + "\nText: " + limitStoryTextLength(plotItem.story.__survey_storyText);
            return tooltipText;
        });
        supportStartingDragOverStoryDisplayItemOrCluster(chartBody, storyDisplayItems);
        function isPlotItemSelected(extent, plotItem) {
    ...
    function d3ContingencyTable(graphBrowserInstance, xAxisQuestion, yAxisQuestion, storiesSelectedCallback) {
        ...
        var chartBody = chart.chartBody;
        // X axis and label
        var xScale = d3.scale.ordinal().domain(columnLabelsArray).rangeRoundBands([0, chart.width], 0.1);
        chart.xScale = xScale;
        chart.xQuestion = xAxisQuestion;
        var xAxis = addXAxis(chart, xScale, { labelLengthLimit: 11, drawLongAxisLines: true, rotateAxisLabels: true });
        addXAxisLabel(chart, nameForQuestion(xAxisQuestion));
        // Y axis and label
        var yScale = d3.scale.ordinal().domain(rowLabelsArray).rangeRoundBands([chart.height, 0], 0.1);
        chart.yScale = yScale;
        chart.yQuestion = yAxisQuestion;
        ...
        var maxPlotItemValue = d3.max(allPlotItems, function (plotItem) {
            return plotItem.value;
        });
        var xValueMultiplier = xScale.rangeBand() / maxPlotItemValue / 2.0;
        var yValueMultiplier = yScale.rangeBand() / maxPlotItemValue / 2.0;
        var storyDisplayClusters = chartBody.selectAll(".storyCluster").data(allPlotItems).enter().append("ellipse").attr("class", "storyCluster observed").attr("rx", function (plotItem) {
            return xValueMultiplier * plotItem.value;
        }).attr("ry", function (plotItem) {
            return yValueMultiplier * plotItem.value;
        }).attr("cx", function (plotItem) {
            return xScale(plotItem.x) + xScale.rangeBand() / 2.0;
        }).attr("cy", function (plotItem) {
            return yScale(plotItem.y) + yScale.rangeBand() / 2.0;
        });
        // Add tooltips
        storyDisplayClusters.append("svg:title").text(function (plotItem) {
